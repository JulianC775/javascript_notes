...
// To use them, copy the entire block for a component (including its imports)
// into a new .tsx file in your React Native project.
...
const MyButton: React.FC<MyButtonProps> = ({ title, onPress, color, style, textStyle, disabled }) => {
    ...
    const textStylesToApply = [styles.text, textStyle];
    if (disabled) {
        textStylesToApply.push(styles.disabledText);
    }
...
export default MyButton; // In a real file, you'd export
*/
...
const styles_ProfileCard = StyleSheet.create({
    cardContainer: {
        backgroundColor: '#fff',
        borderRadius: 8,
        padding: 16,
...
const styles_Counter = StyleSheet.create({
...
// Refer back to the `MyButtonProps` (now commented out) example for `onPress: (event: GestureResponderEvent) => void;`

// --- Example: TextInputHandler.tsx ---
/*
import React, { useState } from 'react';
import { View, TextInput, Text, StyleSheet, NativeSyntheticEvent, TextInputChangeEventData, Button } from 'react-native';

const TextInputHandler: React.FC = () => {
    const [text_TextInputHandler, setText_TextInputHandler] = useState<string>('');
...
const styles_TextInputHandler = StyleSheet.create({
...
/*
import React from 'react';
import { View, Text, StyleSheet, ViewStyle as RNViewStyle, TextStyle as RNTextStyle } from 'react-native';
...
const styles_TypedStyles = StyleSheet.create<ComponentStyles_Typed>({
    container: {
        flex: 1,
        padding: 20,
...
export default TypedStylesComponent;
*/
...
const styles_FlatList = StyleSheet.create({
    header: {
        fontSize: 22,
        fontWeight: 'bold',
        padding: 10,
...
async function fetchPosts_API(): Promise<Post_API[]> {
    try {
        const response = await fetch('https://jsonplaceholder.typicode.com/posts?_limit=5');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data: Post_API[] = await response.json();
...
export type RootStackParamList_Nav = {
    Home_Nav: undefined;
    Profile_Nav: { userId: string; anotherParam?: number };
    Settings_Nav: { settingId: string } | undefined;
};
